...
CObject_Carpet::CObject_Carpet()
{
}

CObject_Carpet::CObject_Carpet(int GHandle, double x, double y, int Width, int Height, unsigned int Type) : CObject_Enemy(GHandle, x, y, Width, Height, Type)
{
	GraphHandle = ResourceManager::getResourceManager()->GetRes(GHandle);
	Stand	= new CSprite(GraphHandle, 6, 0, 0, 6, 1, 1800, 200, 100, TRUE);
	Attack	= new CSprite(GraphHandle, 10, 0, 200,  10, 2, 3000, 200, 75, TRUE);
	Sword	= new CSprite(GetResourceWithKey(IMAGE_ENEMY_STAGE4_SWORD), 5, 0, 0, 5, 1, 600, 120, 100, TRUE);
	Hit		= new CSprite(GraphHandle, 2, 0, 400, 2, 3, 600, 200, 75, TRUE);
	...
	if(GetHP() > 0)
	{
		if(HeroForEnemy->GetWorldX()+100 > AtkX && b_left)
		{
			...
			if(Attack->GetCount() >= 1) {
				SetAttackBox((int)GetDrawX() -(AttackCount*8), (int)GetDrawY()+70, 40, 40);
			}
		}
		else if(HeroForEnemy->GetWorldX()+100 < AtkX && b_right)
		{
			...
			if(Attack->GetCount() >= 1) {
				SetAttackBox((int)GetDrawX()+205 +(AttackCount*8), (int)GetDrawY()+70, 40, 40);
			}
		}
...
void CObject_Carpet::Render()
{
	...
	{
		// 몬스터 HP바 출력
		DrawRectGraph((int)fDrawX+100,	(int)fDrawY+15,	0,	0,	100,									15,		HP_Back,	TRUE, FALSE);
		DrawRectGraph((int)fDrawX+100,	(int)fDrawY+15,	0,	0,	(int)(100*((float)HP/(float)MaxHP)),	173,	HP_Bar,		TRUE, FALSE);
		...
		{
			if(b_left) {
				Sword->DrawSprite((int)fDrawX -(AttackCount*8), (int)fDrawY+30);
				if(Attack->GetCount() >= 8) {
				...
				Sword->DrawSprite((int)fDrawX+170 +(AttackCount*8), (int)fDrawY+30);
				if(Attack->GetCount() >= 8) {
